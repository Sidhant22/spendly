...
- Add "Add Expense" link in navbar visible only when `session["user_id"]` is set
...
- `description`:
  ...
  - store `None` if blank
  - **must not exceed 200 characters (reject with error if longer)**
...
- **Flash message "Expense added successfully" is present**
- New expense row exists in the database
...
`POST /expenses/add` — authenticated, invalid category:

- Returns 200
- Error message shown

`POST /expenses/add` — authenticated, invalid date:
...
`POST /expenses/add` — authenticated, no description:
...
- [ ] Submitting a valid expense redirects to `/profile`
- [ ] **A flash message "Expense added successfully" is shown after redirect**
- [ ] The new expense appears in the transaction list
- [ ] Submitting with a missing or zero amount re-renders the form with an error and preserved values
...
- [ ] Submitting with description longer than 200 characters is rejected with an error
- [ ] Submitting without a description saves successfully with no description
